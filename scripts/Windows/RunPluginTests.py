...
def main() -> int:
    ...
    try:
        ...
        run(f"cmake --build {BUILD_DIR} --target LuAudioPluginTests --parallel 4")
        run(
            f"ctest --test-dir {BUILD_DIR} "
            "-R AudioEffectChainTests^|PluginManagerTests^|PluginIntegrationTests^|WindowsPluginProviderTests "
            "--output-on-failure"
        )
    ...
    print("LuAudio Windows plugin tests passed.")
    return 0
