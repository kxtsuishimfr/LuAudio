...
DEFAULT_BUILD_DIR = PROJECT_ROOT / "out" / "build" / "offline-rendering-tests"
TARGET_NAME = "LuAudioOfflineRenderingTests"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build and run LuAudio offline rendering tests."
    )
    parser.add_argument(
        "--build-dir",
        type=Path,
    ...
    run(
        [
            ...
            "--output-on-failure",
            "-R",
            "OfflineRendererTests|WavFileWriterTests",
        ],
        PROJECT_ROOT,
    )
    return 0
