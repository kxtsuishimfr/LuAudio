...
DEFAULT_AUDIO_FILE = PROJECT_ROOT / "tests" / "Audios" / "WhatsAFutureFunk.wav"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build and play a WAV file through LuAudio WASAPI.")
    ...
    run(
        [
            ...
            str(build_dir),
            "--target",
            "LuAudioWasapiFilePlaybackTest",
        ],
        PROJECT_ROOT,
    )

    executable = build_dir / "LuAudioWasapiFilePlaybackTest.exe"
    if not executable.exists():
        executable = build_dir / "LuAudioWasapiFilePlaybackTest"
    if not executable.exists():
